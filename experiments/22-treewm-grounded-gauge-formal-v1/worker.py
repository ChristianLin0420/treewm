...
def _validated_post_update_cadence(
    payload: Mapping[str, Any], completed_update: int
) -> dict[str, int | str | None]:
    """Require an explicit durable Exp22 cadence boundary without mutating it."""
    ...
    try:
        ...
        raise ContractError(f"checkpoint post-update cadence is invalid: {exc}") from exc
    require(
        cadence.committed_update == completed_update,
        "checkpoint post-update cadence/update boundary differs",
    )
...
def verify_checkpoint(
    ...
    *,
    ...
    post_update_cadence = _validated_post_update_cadence(payload, completed)
    identity = payload.get("run_identity") or {}
    ...
    checks = (
        ...
        identity.get("objective_version") == "treewm_v2_grounded_gauge_formal_v1",
        int(identity.get("total_steps", -1)) == 1_000_000,
        ...
        identity.get("campaign_factorial_arm") == f"exp21-{hashes['selected_arm']}",
    ...
    return {
        ...
        "post_update_cadence": post_update_cadence,
        "checkpoint": str(path),
...
def _require_complete_stage_cadence(
    checkpoint: Mapping[str, Any], target: int
) -> None:
    """Reject a gate marker whose checkpoint still owes post-update work."""
    require(
        checkpoint.get("post_update_cadence")
        == {
            "schema_version": 1,
            "committed_update": target,
            "completed_update": target,
            "replay_action": None,
        },
        "stage checkpoint post-update cadence is incomplete",
    )
...
def verify_stage_marker(run_dir: Path, target: int, launch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require_complete_stage_cadence(checkpoint, target)
    ...
    require(marker.get("objective_version") == "treewm_v2_grounded_gauge_formal_v1", "marker objective differs")
...
def _cancel_before_launch(state_dir: Path, launch: Mapping[str, Any], stage_target: int) -> int:
    run_dir = Path(launch["run"]["run_directory"])
    checkpoint_path = run_dir / "checkpoints" / "latest.pt"
    checkpoint: dict[str, Any] | None = None
    if checkpoint_path.is_file():
        persistent = run_dir / "FORMAL_LAUNCH.json"
        ...
        require(int(checkpoint["completed_updates"]) <= stage_target, "pre-cancel checkpoint passed stage")
    record = {
        "schema_version": 1,
...
def run_worker(args: argparse.Namespace) -> int:
    ...
    if disposition == "cancelled":
        checkpoint_path = run_dir / "checkpoints" / "latest.pt"
        checkpoint = verify_checkpoint(checkpoint_path, launch) if checkpoint_path.is_file() else None
        if checkpoint is not None:
            require(str(checkpoint.get("reason", "")).startswith("graceful-stop:"), "cancellation lacks a graceful checkpoint")
            require(int(checkpoint["completed_updates"]) <= args.stage_target, "cancellation checkpoint passed stage boundary")
        cancelled = {
            "schema_version": 1,
            "status": "cancelled_with_verified_checkpoint" if checkpoint else "cancelled_before_checkpoint",
            "stage_target": args.stage_target,
    ...
    if disposition == "requeue":
        ...
        require(int(record["completed_updates"]) <= args.stage_target, "preemption checkpoint passed stage without marker")
