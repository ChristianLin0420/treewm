...
def prepare_source_snapshot(repo_root: Path, manifest: Mapping[str, Any]) -> Path:
    ...
    with (parent / ".create.lock").open("a+", encoding="utf-8") as lock:
        ...
        try:
            ...
            copied_protocol = verify_protocol_lock(temporary / "experiments" / "22-treewm-grounded-gauge-formal-v1")
...
def test_only_report(manifest: Mapping[str, Any], repo_root: Path) -> dict[str, Any]:
    """Validate the inert package without requiring unavailable prerequisite outputs."""
    package = repo_root / "experiments" / "22-treewm-grounded-gauge-formal-v1"
    validate_slurms(package)
    protocol = verify_protocol_lock(package)
    seed_table = load_seed_table(manifest, package / "eval_seed_table.json")
    binding = read_json(package / "prerequisite_bindings.json")
    sealed = binding.get("status") == "sealed_accepted_exp20_and_exp21_raw_recomputed"
    report: dict[str, Any] = {
        "schema_version": 1,
        "status": (
            "verified_submission_inert"
            if sealed
            else "blocked_waiting_for_accepted_exp20_and_exp21_raw_evidence"
        ),
        ...
        "package_protocol_sha256": protocol,
        "prerequisite_binding_status": binding.get("status"),
        "evaluation_seed_table_sha256": seed_table["sha256"],
        "training_runs": len(expand_runs(manifest)),
        "final_eval_tasks": manifest["expected_final_eval_tasks"],
        "namespace_fresh": namespace_is_fresh(manifest),
    }
    if sealed:
        report["verification"] = verify_all(
            manifest, repo_root=repo_root, verify_files=False
        )
    return report
...
def launch_plan(
    ...
    *,
    verify_files: bool,
    recompute_raw_prerequisites: bool = True,
    expected_raw_recomputation_sha256: str | None = None,
) -> dict[str, Any]:
    package = repo_root / "experiments" / "22-treewm-grounded-gauge-formal-v1"
    validate_slurms(package)
    sealed_binding = read_json(package / "prerequisite_bindings.json")
    if recompute_raw_prerequisites:
        # This is the sole scientific authorization path: independently replay
        # Exp20+Exp21 raw evidence and require byte-identical equality to the
        # protocol-bound local receipt.  It runs once, never in array workers.
        from bind_prerequisites import build_binding

        recomputed_binding = build_binding(manifest)
        require(
            recomputed_binding == sealed_binding,
            "sealed prerequisite receipt is not exactly reproducible from raw Exp20+Exp21 evidence",
        )
        raw_recomputation_sha256 = stable_hash(recomputed_binding)
    else:
        require(
            expected_raw_recomputation_sha256 is not None,
            "snapshot launch plan lacks the live raw-prerequisite preflight receipt",
        )
        raw_recomputation_sha256 = stable_hash(sealed_binding)
        require(
            raw_recomputation_sha256 == expected_raw_recomputation_sha256,
            "snapshot prerequisite receipt differs from live raw-recomputed preflight",
        )
    verification = verify_all(
        manifest,
        repo_root=repo_root,
        verify_files=verify_files,
        # build_binding() just hashed and parsed every raw artifact on the live
        # preflight.  The immutable snapshot pass rehashes the sealed inventory
        # once more to close the preflight-to-submit mutation window.
        verify_prerequisite_files=not recompute_raw_prerequisites,
    )
    ...
    plan: dict[str, Any] = {
        ...
        "raw_prerequisite_recomputation_sha256": raw_recomputation_sha256,
...
def _submit_node(
    ...
    *,
    ...
    command.extend([f"--export=ALL,{rendered_exports}", f"--output={output}", str(repo_root / "experiments" / "22-treewm-grounded-gauge-formal-v1" / script)])
...
def submit_dag(manifest: Mapping[str, Any], repo_root: Path, plan: Mapping[str, Any], state_root: Path) -> dict[str, Any]:
    ...
    exports = {
        "TREEWM_GAUGE_FORMAL_V1_REPO_ROOT": str(repo_root),
        ...
        "TREEWM_EXPECTED_PREREQUISITE_BINDING_SHA256": common["prerequisite_binding_sha256"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--test-only",
        action="store_true",
        help="validate the inert package without snapshotting, scheduler access, or submission",
    )
    args = parser.parse_args(argv)
    if args.submit and args.test_only:
        parser.error("--submit and --test-only are mutually exclusive")
    live_root = args.repo_root.resolve()
    manifest = load_manifest(args.manifest)
    verify_submit_interpreter(manifest)
    if args.test_only:
        print(json.dumps(test_only_report(manifest, live_root), sort_keys=True, indent=2))
        return 0
    ...
    dry_plan = launch_plan(
        ...
        recompute_raw_prerequisites=True,
    )
    ...
    try:
        ...
        snapshot_manifest = load_manifest(snapshot / "experiments" / "22-treewm-grounded-gauge-formal-v1" / "manifest.json")
        ...
        plan = launch_plan(
            ...
            snapshot,
            verify_files=True,
            recompute_raw_prerequisites=False,
            expected_raw_recomputation_sha256=dry_plan[
                "raw_prerequisite_recomputation_sha256"
            ],
        )
