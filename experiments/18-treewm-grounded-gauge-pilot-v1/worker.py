#!/usr/bin/env python3
"""Run one staged gauge-pilot slot with exact resume and durable signal handling."""
...
def verify_checkpoint(
    ...
    *,
    ...
    checks = (
        ...
        identity.get("objective_version") == "treewm_v2_grounded_gauge_pilot_v1",
...
def verify_stage_marker(run_dir: Path, target: int, launch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    require(marker.get("objective_version") == "treewm_v2_grounded_gauge_pilot_v1", "marker objective differs")
...
def _cancel_before_launch(
    ...
    if checkpoint_path.is_file():
        persistent = run_dir / "GAUGE_PILOT_LAUNCH.json"
...
def run_worker(args: argparse.Namespace) -> int:
    ...
    persistent_launch = run_dir / "GAUGE_PILOT_LAUNCH.json"
    ...
    if disposition == "requeue":
        checkpoint = verify_checkpoint(run_dir / "checkpoints" / "latest.pt", launch)
        require(int(checkpoint["completed_updates"]) < args.stage_target, "preemption checkpoint reached/passed stage without marker")
