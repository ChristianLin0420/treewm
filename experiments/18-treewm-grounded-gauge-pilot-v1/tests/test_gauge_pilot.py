...
def _load_local_modules():
    names = ("campaign", "worker", "stage_gate", "submit")
    ...
    try:
        ...
        return campaign, worker, stage_gate, submit
    finally:
...
campaign, worker, stage_gate, submit = _load_local_modules()
...
def test_exact_stage_mappings_and_fresh_seeds(manifest):
    ...
    assert {run.seed for run in runs} == {104, 105}
    ...
    assert {run.seed for run in continuation} == {104, 105}
...
def test_n_g_gs_change_only_preregistered_gauge_and_clip_switches(manifest):
    ...
    invariants = (
        "experiment=treewm_v2_grounded_gauge_pilot",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda value: value["engineering_diagnosis"].update(accepted_prerequisite=True),
        lambda value: value["engineering_diagnosis"].update(resume_allowed=True),
    ],
)
...
def _fake_launch(run):
    ...
    launch = {
        "campaign_id": "treewm-grounded-gauge-pilot-v1",
...
def _mock_stage5(monkeypatch, manifest, *, g_pass=True, gs_pass=True, causal=True):
    monkeypatch.setattr(
        stage_gate,
        "trainer_command",
...
def test_25k_outcome_requires_replication_by_seed_and_setting(manifest):
    ...
    assert result["per_seed"]["104"]["successes"] == 1.0
    assert result["per_seed"]["105"]["successes"] == 1.0
    ...
    with pytest.raises(campaign.ContractError, match="seed 105"):
...
def test_skip_artifact_is_hash_bound_and_idempotent(monkeypatch, tmp_path):
    run = type("Run", (), {
        ...
        "arm_id": "G",
        "seed": 104,
        "run_name": "gauge-antmaze-large-armg-seed104",
    })()
...
def test_previous_gate_is_immutable_and_selects_per_launch(monkeypatch, tmp_path, manifest):
    changed = copy.deepcopy(manifest)
    changed["paths"]["run_root"] = str(tmp_path / "treewm-grounded-gauge-pilot-v1")
...
def test_protocol_lock_slurms_and_namespace_are_isolated(manifest, tmp_path):
    ...
    submit.validate_slurms(CAMPAIGN_DIR)
    assert manifest["paths"]["run_root"].endswith("/outputs/treewm-grounded-gauge-pilot-v1")
    changed = copy.deepcopy(manifest)
    changed["paths"]["run_root"] = str(tmp_path / "fresh")
    assert submit.namespace_is_fresh(changed) is True
    launch = tmp_path / "fresh" / "x" / "GAUGE_PILOT_LAUNCH.json"
