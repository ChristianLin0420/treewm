#!/usr/bin/env python3
"""Dry-run or explicitly submit the sealed exp18 staged gauge-pilot DAG."""
    ...
    with (parent / ".create.lock").open("a+", encoding="utf-8") as lock:
        ...
        try:
            ...
            copied_protocol = verify_protocol_lock(
                temporary / "experiments" / "18-treewm-grounded-gauge-pilot-v1"
            )
...
def launch_plan(
    ...
    *,
    ...
    package = repo_root / "experiments" / "18-treewm-grounded-gauge-pilot-v1"
    ...
    plan: dict[str, Any] = {
        "schema_version": 1,
        "status": "sealed_bounded_causal_gauge_pilot_plan",
...
def _submit_node(
    ...
    *,
    ...
    command.extend([
        ...
        f"--output={output}",
        str(repo_root / "experiments" / "18-treewm-grounded-gauge-pilot-v1" / script),
    ])
    result = subprocess.run(
        command,
        cwd=repo_root,
...
def submit_dag(
    ...
    base = {
        "TREEWM_GAUGE_PILOT_REPO_ROOT": str(repo_root),
    ...
    receipt: dict[str, Any] = {
        "schema_version": 1,
        "status": "staged_causal_gauge_pilot_submitted",
        "campaign_id": manifest["campaign_id"],
        "formal_validation": False,
        "plan_sha256": plan["plan_sha256"],
        "repo_root": str(repo_root),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not namespace_is_fresh(manifest):
        raise ContractError("fresh exp18 namespace already contains launch/checkpoint/gate/receipt state")
    ...
    if receipt_path.exists():
        raise ContractError("exp18 already has a submission receipt")
    create_claim(claim, {
        "schema_version": 1,
    ...
    try:
        ...
        snapshot_manifest = load_manifest(
            snapshot / "experiments" / "18-treewm-grounded-gauge-pilot-v1" / "manifest.json"
        )
...
if __name__ == "__main__":
    try:
        ...
        print(f"gauge-pilot submit error: {exc}", file=sys.stderr)
        raise SystemExit(2)
