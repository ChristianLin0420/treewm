#!/usr/bin/env python3
"""Recover a committed-but-unflushed Exp21 MetricTracker log boundary.
...
the exact-resume checkpoint contains the complete MetricTracker window, but the
TensorBoard point for that boundary has not yet been published. This module makes
...
def _write_event(
    ...
    writer = SummaryWriter(
        log_dir=str(run_dir),
        filename_suffix=f".exp21-metric-boundary-{step}",
        flush_secs=1,
    )
    try:
