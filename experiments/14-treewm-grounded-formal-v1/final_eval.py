...
def run_final_eval(args: argparse.Namespace) -> int:
    ...
    seed_bundle = load_seed_table(manifest, root / "experiments" / "14-treewm-grounded-formal-v1" / "eval_seed_table.json")
    ...
    if progress_path.exists():
        ...
        if progress.get("status") != "in_progress" or progress.get("identity") != identity:
    ...
    def publish_requeue_ready() -> None:
        ...
        if current.get("identity") != identity or current.get("status") != "in_progress":
            raise ContractError("final-eval progress is not exact before requeue")
        record = {
            "schema_version": 1,
            "status": "final_eval_progress_verified_for_requeue",
            "eval_contract_sha256": identity["eval_contract_sha256"],
            "progress_sha256": stable_hash(current),
            "unix_time": time.time(),
        }
