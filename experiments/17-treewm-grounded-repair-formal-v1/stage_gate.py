...
def evaluate_metrics(manifest: Mapping[str, Any], metrics: Mapping[str, Mapping[int, float]], target: int) -> dict[str, Any]:
    ...
    last = {
        tag: sample[1] if sample is not None else None
        for tag, sample in samples.items()
    }
    last_step = {
        tag: sample[0] if sample is not None else None
        for tag, sample in samples.items()
    }
    expected_last_step = {
        tag: target if tag in training_tags else expected_validation_step
        for tag in required
    }
    finite_coverage = all(finite(value) for value in last.values())
    target_appropriate_telemetry = all(
        last_step[tag] == expected_last_step[tag] for tag in required
    )
    fixed_counts = _values(metrics, "data/validation_fixed_sample_count", target)
    fixed_validation = bool(
        fixed_counts
        and all(finite(value) for value in fixed_counts)
        and len({float(value) for value in fixed_counts}) == 1
        and fixed_counts[0] > 0
    )
    gradient_window = min(int(gate["gradient_recent_window_updates"]), target)
    training_cadence = int(freshness["training_every_updates"])
    expected_gradient_axis = _expected_axis(target, training_cadence, gradient_window)
    gradient_tags = (
    ...
    gradient_series = {
        tag: {
            ...
            for step, value in metrics.get(tag, {}).items()
            if expected_gradient_axis[0] <= step <= target
        }
        for tag in gradient_tags
    }
    complete_gradient_axis = bool(
        expected_gradient_axis
        and all(tuple(sorted(values)) == expected_gradient_axis for values in gradient_series.values())
    )
    world_norm = list(gradient_series["train/grad_norm_world"].values())
    gain_norm = list(gradient_series["train/grad_norm_gain"].values())
    world_clip = list(gradient_series["train/grad_clip_coefficient_world"].values())
    gain_clip = list(gradient_series["train/grad_clip_coefficient_gain"].values())
    norm_values = [*world_norm, *gain_norm]
    clip_values = [*world_clip, *gain_clip]
    gradient_pass = bool(
        complete_gradient_axis
        and all(finite(value) and value > float(gate["min_gradient_norm"]) for value in norm_values)
    )
    low_clip_fraction = (
        sum(value < float(gate["min_clip_coefficient"]) for value in clip_values)
        / len(clip_values)
        if clip_values and all(finite(value) for value in clip_values)
        else None
    )
    clipping_pass = bool(
        complete_gradient_axis
        and clip_values
        and all(finite(value) and 0.0 < value <= 1.0 for value in clip_values)
    ...
    validation_stable = bool(
        val_values
        and all(finite(value) for value in val_values)
    ...
    self_fed_multistep_stable = bool(
        ...
        and all(finite(value) for value in self_fed_multistep_values)
    ...
    recent_mean = {
        ...
        if values and all(finite(value) for value in values)
        else None
        for tag, values in recent.items()
    }
    q_pass = bool(
        not enforce
        or (
    ...
    integrity_gates = {
        ...
        "target_appropriate_telemetry": target_appropriate_telemetry,
        "fixed_validation_sample": fixed_validation,
        "complete_recent_gradient_telemetry_axis": complete_gradient_axis,
        "nonzero_world_and_gain_gradients": gradient_pass,
        "bounded_world_and_gain_gradient_clipping": clipping_pass,
    }
    scientific_gates = {
    ...
    return {
        ...
        "recent_gradient_expected_sample_count": len(expected_gradient_axis),
        "recent_gradient_sample_count": {
            tag: len(values) for tag, values in gradient_series.items()
        },
