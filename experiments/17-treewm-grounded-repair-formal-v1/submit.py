...
def prepare_source_snapshot(repo_root: Path, manifest: Mapping[str, Any]) -> Path:
    ...
    with (parent / ".create.lock").open("a+", encoding="utf-8") as lock:
        ...
        try:
            ...
            copied_protocol = verify_protocol_lock(temporary / "experiments" / "17-treewm-grounded-repair-formal-v1")
...
def test_only_report(manifest: Mapping[str, Any], repo_root: Path) -> dict[str, Any]:
    ...
    package = repo_root / "experiments" / "17-treewm-grounded-repair-formal-v1"
    ...
    sealed = binding.get("status") == "sealed_accepted_prerequisites"
    report: dict[str, Any] = {
        "schema_version": 1,
        "status": (
            "verified_submission_inert"
            if sealed
            else "blocked_waiting_for_accepted_exp15_and_exp16"
        ),
...
def _submit_node(
    ...
    *,
    ...
    command.extend([f"--export=ALL,{rendered_exports}", f"--output={output}", str(repo_root / "experiments" / "17-treewm-grounded-repair-formal-v1" / script)])
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        snapshot_manifest = load_manifest(snapshot / "experiments" / "17-treewm-grounded-repair-formal-v1" / "manifest.json")
