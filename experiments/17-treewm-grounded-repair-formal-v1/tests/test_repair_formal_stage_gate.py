...
def healthy_metrics(step: int = 25_000) -> dict[str, dict[int, float]]:
    ...
    freshness = manifest["stage_acceptance"]["telemetry_freshness"]
    training_tags = set(freshness["training_exact_target_tags"])
    boundary_every = int(freshness["validation_diagnostic_every_updates"])
    boundary_step = step - (step % boundary_every)
    metrics = {
        tag: {boundary_step: value}
        for tag, value in values.items()
        if tag not in training_tags
    }
    gradient_window = min(
        int(manifest["stage_acceptance"]["gradient_recent_window_updates"]), step
    )
    training_axis = stage_gate._expected_axis(
        step, int(freshness["training_every_updates"]), gradient_window
    )
    metrics.update(
        {
            tag: {axis_step: value for axis_step in training_axis}
            for tag, value in values.items()
            if tag in training_tags
        }
    )
    return metrics
...
def test_horizon_and_gain_coverage_gate_passes() -> None:
    ...
    assert report["clip_fraction_below_threshold"] == 0.0
...
def test_scientific_gate_rejects_known_degenerate_metrics(tag: str, value: float) -> None:
    ...
    metrics[tag][max(metrics[tag])] = value
...
def test_self_fed_multistep_validation_is_independently_nonregressing() -> None:
    metrics = healthy_metrics()
    metrics["val/loss_multistep_self_fed"] = {20_000: 0.5, 24_000: 0.7}
    report = stage_gate.evaluate_metrics(campaign.load_manifest(), metrics, 25_000)
    assert report["integrity_passed"]
...
def test_newer_nonfinite_scalar_cannot_fall_back_to_older_finite_value() -> None:
    metrics = healthy_metrics()
    metrics["val/loss_total"] = {22_000: 1.0, 24_000: float("nan")}
    report = stage_gate.evaluate_metrics(campaign.load_manifest(), metrics, 25_000)
    assert not report["integrity_gates"]["required_finite_telemetry"]
    assert not report["scientific_gates"]["validation_nonregression"]
    assert not report["integrity_passed"]


def test_1m_gate_rejects_stale_2k_telemetry() -> None:
    report = stage_gate.evaluate_metrics(
        campaign.load_manifest(), healthy_metrics(2_000), 1_000_000
    )
    assert not report["integrity_gates"]["target_appropriate_telemetry"]
    assert not report["integrity_gates"]["complete_recent_gradient_telemetry_axis"]
    assert not report["integrity_passed"]


def test_25k_gate_accepts_the_preregistered_24k_validation_boundary() -> None:
    report = stage_gate.evaluate_metrics(
        campaign.load_manifest(), healthy_metrics(25_000), 25_000
    )
    assert report["integrity_gates"]["target_appropriate_telemetry"]
    assert report["last_step"]["val/loss_total"] == 24_000
    assert report["last_step"]["train/grad_norm_world"] == 25_000


def test_missing_recent_gradient_sample_fails_closed() -> None:
    metrics = healthy_metrics()
    metrics["train/grad_clip_coefficient_gain"].pop(22_500)
    report = stage_gate.evaluate_metrics(campaign.load_manifest(), metrics, 25_000)
    assert not report["integrity_gates"]["complete_recent_gradient_telemetry_axis"]
    assert not report["integrity_passed"]


@pytest.mark.parametrize("value", [float("nan"), float("inf"), 0.0, 1.01])
def test_clip_coefficients_must_be_finite_and_in_unit_interval(value: float) -> None:
    metrics = healthy_metrics()
    metrics["train/grad_clip_coefficient_world"][25_000] = value
    report = stage_gate.evaluate_metrics(campaign.load_manifest(), metrics, 25_000)
    assert not report["integrity_gates"]["bounded_world_and_gain_gradient_clipping"]
    assert not report["integrity_passed"]


def test_1m_gate_rejects_low_clip_saturation() -> None:
    metrics = healthy_metrics(1_000_000)
    for tag in (
        "train/grad_clip_coefficient_world",
        "train/grad_clip_coefficient_gain",
    ):
        metrics[tag] = {step: 0.01 for step in metrics[tag]}
    report = stage_gate.evaluate_metrics(campaign.load_manifest(), metrics, 1_000_000)
    assert report["integrity_gates"]["target_appropriate_telemetry"]
    assert report["clip_fraction_below_threshold"] == 1.0
    assert not report["integrity_gates"]["bounded_world_and_gain_gradient_clipping"]
    assert not report["integrity_passed"]
...
def test_scientific_coverage_is_three_of_four_per_setting() -> None:
    ...
    for setting in ("scene", "cube-double"):
        rows.extend(
            {"setting_id": setting, "health": {"scientific_passed": seed_index < 3}}
            for seed_index, _seed in enumerate(campaign.SEEDS)
        )
...
def test_100k_outcome_sanity_rejects_all_zero_fleet() -> None:
    ...
    assert stage_gate.validate_outcome_sanity(manifest, outcome_rows(0.0), 1_000_000) is None
...
def test_100k_outcome_sanity_requires_positive_progress() -> None:
    manifest = campaign.load_manifest()
    rows = outcome_rows(1.0)
    for row in rows:
        row["monitor"]["distance_reduction_frac"] = 0.0
    with pytest.raises(campaign.ContractError, match="mean progress"):
        stage_gate.validate_outcome_sanity(manifest, rows, 100_000)


def test_method_quorum_is_only_the_preregistered_25k_gate() -> None:
    manifest = campaign.load_manifest()
    rows = [
        {"setting_id": setting, "health": {"scientific_passed": False}}
        for setting in campaign.SETTING_IDS
        for _seed in campaign.SEEDS
    ]
    assert stage_gate.validate_scientific_setting_coverage(manifest, rows, 100_000) is None
    assert stage_gate.validate_scientific_setting_coverage(manifest, rows, 1_000_000) is None
