...
def wrap_envs(env, eval_env, frame_stack=None, agent_config=None):
    """Apply the same monitoring/chunking wrappers used by the upstream loader."""
...
def make_env_and_datasets(
    ...
    if 'singletask' in env_name:
        ...
        if dataset_dir is not None:
            dataset_kwargs['dataset_dir'] = os.path.expanduser(dataset_dir)
        env, train_dataset, val_dataset = ogbench.make_env_and_datasets(env_name, **dataset_kwargs)
        eval_env = ogbench.make_env_and_datasets(env_name, env_only=True, **dataset_kwargs)
    ...
    env, eval_env = wrap_envs(env, eval_env, frame_stack=frame_stack, agent_config=agent_config)
