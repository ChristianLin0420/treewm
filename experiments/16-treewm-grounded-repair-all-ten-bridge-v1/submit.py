#!/usr/bin/env python3
"""Dry-run or explicitly submit the sealed 40-task all-ten bridge and report."""
...
def validate_slurms(package: Path) -> None:
    bridge = _exact_lines(package / "bridge.slurm", BRIDGE_LINES, "bridge Slurm")
    report = _exact_lines(package / "report.slurm", REPORT_LINES, "report Slurm")
    if "TREEWM_PYTHON" in bridge or "TREEWM_PYTHON" in report:
    ...
    for snippet in (
        ...
        if snippet not in bridge:
            raise ContractError(f"bridge Slurm lacks {snippet!r}")
    for snippet in (
        'campaign.py" snapshot',
        "report.py",
        "--publish",
    ...
    with (parent / ".create.lock").open("a+", encoding="utf-8") as lock:
        ...
        try:
            ...
            copied_protocol = verify_protocol_lock(
                temporary / "experiments" / "16-treewm-grounded-repair-all-ten-bridge-v1"
            )
...
def namespace_is_fresh(manifest: Mapping[str, Any]) -> bool:
    ...
    forbidden = []
    if run_root.exists():
        forbidden.extend(run_root.rglob("BRIDGE_LAUNCH.json"))
...
def launch_plan(
    ...
    *,
    ...
    package = repo_root / "experiments" / "16-treewm-grounded-repair-all-ten-bridge-v1"
    ...
    if (
        verification["exp15_prerequisite"]["prerequisite_sha256"]
        != common["exp15_prerequisite_sha256"]
    ):
        raise ContractError("exp15 prerequisite changed during launch-plan sealing")
    plan: dict[str, Any] = {
        "schema_version": 1,
        "status": "sealed_bounded_all_ten_bridge_plan",
...
def _submit_node(
    ...
    *,
    ...
    command.extend([
        ...
        f"--output={output}",
        str(repo_root / "experiments" / "16-treewm-grounded-repair-all-ten-bridge-v1" / script),
    ])
    result = subprocess.run(
        command,
        cwd=repo_root,
...
def submit_dag(
    ...
    exports = {
        "TREEWM_BRIDGE_REPO_ROOT": str(repo_root),
        ...
        "TREEWM_EXPECTED_EXP15_PREREQUISITE_SHA256": common[
            "exp15_prerequisite_sha256"
        ],
    }
    logs = Path(manifest["paths"]["run_root"]) / "logs"
    jobs: dict[str, Any] = {}
    bridge_id, command, stdout, stderr = _submit_node(
        manifest,
        repo_root=repo_root,
        script="bridge.slurm",
        exports=exports,
        output=logs / "bridge_%A_%a.out",
        dependency=None,
    )
    jobs["bridge"] = {"job_id": bridge_id, "dependency": None, "command": command, "stdout": stdout, "stderr": stderr}
    atomic_json(state_root / "SUBMISSION_PROGRESS.json", {
        "schema_version": 1,
        "status": "bridge_submitted_report_pending",
    ...
    report_id, command, stdout, stderr = _submit_node(
        ...
        dependency=bridge_id,
    )
    jobs["strict_report"] = {"job_id": report_id, "dependency": bridge_id, "command": command, "stdout": stdout, "stderr": stderr}
    receipt: dict[str, Any] = {
        "schema_version": 1,
        "status": "bounded_all_ten_bridge_and_report_submitted",
        "campaign_id": manifest["campaign_id"],
        "formal_validation": False,
        "plan_sha256": plan["plan_sha256"],
        "repo_root": str(repo_root),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not namespace_is_fresh(manifest):
        raise ContractError("fresh bridge namespace already contains a launch/checkpoint/report/receipt")
    ...
    if receipt_path.exists():
        raise ContractError("all-ten bridge already has a submission receipt")
    create_claim(claim, {
        "schema_version": 1,
    ...
    try:
        ...
        snapshot_manifest = load_manifest(
            snapshot / "experiments" / "16-treewm-grounded-repair-all-ten-bridge-v1" / "manifest.json"
        )
        plan = launch_plan(
            snapshot_manifest,
            snapshot,
...
if __name__ == "__main__":
    try:
        ...
        print(f"all-ten bridge submit error: {exc}", file=sys.stderr)
        raise SystemExit(2)
