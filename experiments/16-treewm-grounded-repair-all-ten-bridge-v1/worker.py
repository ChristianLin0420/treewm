#!/usr/bin/env python3
"""Run one all-ten bridge model with durable cancellation and exact requeue checks."""
...
def verify_checkpoint(path: Path, launch: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(payload, Mapping):
        raise ContractError("checkpoint payload is not an object")
    ...
    checks = (
        ...
        == hashes["exp15_prerequisite_sha256"],
...
def run_worker(args: argparse.Namespace) -> int:
    ...
    persistent_launch = run_dir / "BRIDGE_LAUNCH.json"
...
if __name__ == "__main__":
    try:
        ...
        print(f"all-ten bridge worker error: {exc}", file=sys.stderr, flush=True)
        raise SystemExit(2)
