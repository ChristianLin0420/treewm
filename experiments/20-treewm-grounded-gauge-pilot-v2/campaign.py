#!/usr/bin/env python3
"""Fail-closed contracts and deterministic launch mapping for gauge pilot 20."""
...
def validate_manifest(manifest: Mapping[str, Any]) -> None:
    ...
    require(superseded.get("package") == "experiments/18-treewm-grounded-gauge-pilot-v1", "superseded package drifted")
    require(
        superseded.get("package_protocol_sha256")
        == "1bfb936a7179a33eb813224db9817fffa476503b42788dbbf328a36cc7260c14",
        "superseded protocol identity drifted",
    )
    require(superseded.get("status") == "superseded_nonpromotable_control_gate_error", "root-cause status drifted")
    require("nonpromotable N" in str(superseded.get("root_cause", "")), "root-cause statement drifted")
    require(superseded.get("results_consumed") is False, "Exp18 results became an input")
    require(superseded.get("checkpoints_consumed") is False, "Exp18 checkpoints became an input")
    require(superseded.get("resume_allowed") is False, "Exp18 resume became eligible")
    require(superseded.get("design_provenance_only") is True, "Exp18 provenance is not design-only")

    method = manifest.get("method") or {}
    require(method == {
        "arm": "treewm",
        "model_class": "TreeWM",
        "experiment_config": "treewm_v2_grounded_gauge_pilot_v2",
    ...
    fresh_start = str(design.get("fresh_start_policy", ""))
    require("start from scratch" in fresh_start and "Exp18 results and checkpoints" in fresh_start, "fresh start guard missing")
    ...
    require("completed boundary wins" in cancellation, "completion/cancellation precedence missing")
    metric_boundary = str(lifecycle.get("metric_boundary_policy", ""))
    require("50-update boundary" in metric_boundary and "resets only the tracker window" in metric_boundary, "MetricTracker recovery policy missing")
    ...
    control_policy = str(gate.get("nonpromotable_structural_gate_policy", ""))
    require("N must pass" in control_policy and "cannot block selection" in control_policy, "N structural-only policy missing")
...
def verify_source_snapshot(repo_root: str | Path) -> dict[str, Any]:
    ...
    protocol = verify_protocol_lock(root / "experiments" / "20-treewm-grounded-gauge-pilot-v2")
...
def actual_evaluation_bank(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # This bounded campaign stops behind the 25k gate and consumes the periodic
    # one-episode-per-task monitor, never the five-episode terminal-eval bank.
...
def verify_all(
    ...
    *,
    ...
    return {
        "schema_version": 1,
        "status": "verified_bounded_causal_gauge_pilot_v2",
        "formal_validation": False,
        "campaign_id": manifest["campaign_id"],
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "protocol-hash":
        print(protocol_sha256(root / "experiments" / "20-treewm-grounded-gauge-pilot-v2"))
        return 0
